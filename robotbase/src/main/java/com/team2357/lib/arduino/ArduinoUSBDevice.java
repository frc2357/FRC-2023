...
public class ArduinoUSBDevice {
    private static class SerialPortHandler implements Runnable{
        ...
        public void run(){
            while(m_thread!=null){
                int bytesAvailible = m_serialPort.bytesAvailable();
                sendKeepAlive();
                while(bytesAvailible==0 && m_thread!=null){
                    try{
        ...
        private void read(int bytesAvailible) throws IOException{
            int bytesToRead = Math.min(bytesAvailible, BYTE_BUFFER_SIZE);
            System.out.println(bytesToRead);
            int byteCount = m_serialPort.readBytes(m_byteBuffer,bytesToRead);
            if(byteCount==0){
                return;
            }
